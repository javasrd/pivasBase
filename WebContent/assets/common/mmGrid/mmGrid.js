...
!function($){
    ...
    MMGrid.prototype = {
        ...
        , _loadAjax: function(args){
            ...
            if(opts.dataType && opts.dataType=="form"){//如果非 json 则传dataType=form，否则默认改为json方式，且参数JSON.stringify(params)
            ...
            $.ajax({
                ...
                try{
                    ...
                    if(data && $.isArray(data[opts.root])){
                    	//data = $.extend(args, data);//这句话 会把查询出来的结果list 也放回到入参里面去，所以注释掉
                    }
        ...
        , loadInit: function(args){
            if(args){
                this.opts.params = args ;
        ...
        , selectedValsByName: function(filedName){
       	 var $body = this.$body;
            var selected = [];
            var row ;
            $.each($body.find('tr.selected'), function(index ,item){
           	 row = $.data(this,'item');
           	 selected.push(row[filedName]);
            });
          return selected;
        }
