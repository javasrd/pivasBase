...
$.fn.extend({ 
	...
	var op = $.extend({ wraperClass: "wraper", data: "", selected: "", selectAll:"全部"}, options); 
	...
	return this.each(function () { 
		...
		var $wraper = $(conSelector).wrapAll("<span><div><div></div></div></span>").parent().parent().addClass(op.wraperClass); 
		...
		//加载默认数据 
		$list.append('<ul><li><input type="checkbox" class="selectAll" value="" /><span>'+op.selectAll+'</span></li></ul>'); 
			var $ul = $list.find("ul"); 
			//加载json数据 
			var listArr = JSON.parse(op.data); 
			var jsonData; 
